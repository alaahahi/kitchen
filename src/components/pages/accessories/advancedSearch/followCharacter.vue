<template>
  <div>
    <button
      v-if="user && !storedSearches.includes(Number(id))"
      @click="postFollowcharactour(id)"
    ...
    <button
      v-if="user && storedSearches.includes(Number(id))"
      @click="postUnFollowcharactour(id)"
      class="btn px-3 fs-16 btn-outline-primary resultsMost"
    >
      <svg width="22" height="24" viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
...
export default {
  ...
  computed: {
    storedSearches: function () {
      return localStorage.followed_characters === undefined ? [] : [...JSON.parse(localStorage.followed_characters)]
    },
  },
  methods: {
    onEnter(id) {
      ...
      if (!storedSearches.includes(id)) {
        ...
        localStorage.followed_characters = JSON.stringify(storedSearches)
      }
    },
    onUnEnter(id) {
      let storedSearches = this.storedSearches
      ...
      this.http
        ...
        .catch((error) => {
          if (error.response.data.errorCode == 402) {
            this.$toast.warning('يجب عليك اكمال معلوماتك أولا لمتابعة الشخصية')
          } else {
